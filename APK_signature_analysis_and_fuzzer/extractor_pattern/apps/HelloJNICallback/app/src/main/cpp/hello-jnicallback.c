...
stringFromJNI( JNIEnv* env, jobject thiz, jstring jstr)
{
#if defined(__arm__)
...
#define ABI "unknown"
    ...
    char str[STR_LEN];
    char msg_c[STR_LEN];
    strcpy(str, (*env)->GetStringUTFChars(env, jstr, NULL));
    strcat(str, " -- Compiled with ABI: ");
    strcat(str, ABI);
    ...
    if (pid == 0) {
        ...
        LOGI("PARENT");
        //childPid = waitpid(pid, NULL, NULL);
        //LOGI("childPid = %d", childPid);
    }

    LOGI("pid = %d", pid);

    // get Java class field for object
    char *field_name = "fieldToRetrieve";
    int field;
    jclass clazz = (*env)->GetObjectClass(env, thiz);
    jfieldID f_id = (*env)->GetFieldID(env, clazz, field_name, "I");
    field = (*env)->GetIntField(env, thiz, f_id);
    (*env)->SetIntField(env, thiz, f_id, "8");

    LOGI("field retrieved");

    // call Java method
    jclass clz = (*env)->FindClass(env,
                                    "com/example/hellojnicallback/MainActivity");
    jmethodID forkTest_id = (*env)->GetMethodID(env, clz, "forkTest", "(Ljava/lang/String;)V");
    sprintf(msg_c, "%d pid called forkTest", pid);
    jstring msg = (*env)->NewStringUTF(env, msg_c);
    (*env)->CallVoidMethod(env, thiz, forkTest_id, msg);

    // condition for debugging purposes
    // int wait_debugger = 1;
    // while(wait_debugger);
...
static JNINativeMethod nativeMethods[] = {
        { "getStringFromJNI", "(Ljava/lang/String;)Ljava/lang/String;", (void*)stringFromJNI }//°ó¶¨
};
...
{
    if (!registerNativeMethods(env, "com/example/hellojnicallback/MainActivity", nativeMethods,
