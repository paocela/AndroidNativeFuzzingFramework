...
extern "C"
{
	...
	/***********************************/
    typedef jbyteArray function_t(JNIEnv *, jobject, jbyteArray, jbyteArray);
}
...
int main(int argc, char *argv[]) {
		...
		value_native = (char *) malloc(value.length() * sizeof(char));
		strcpy(value_native, value.c_str());

		jvalue = env->NewByteArray(value.length());
		env->SetByteArrayRegion(jvalue, 0,  value.length(), (jbyte *)value_native);

		// read key
		std::getline(std::cin, key);
		key_native = (char *) malloc(key.length() * sizeof(char));
		strcpy(key_native, key.c_str());

		jkey = env->NewByteArray(key.length());
		env->SetByteArrayRegion(jkey, 0,  key.length(), (jbyte *)key_native);

		std::cout << value << " " << key << std::endl;
      
        /* Call target function -- Fuzz */
        targetFunctionPtr(env, MainActivityCls, jvalue, jkey);
