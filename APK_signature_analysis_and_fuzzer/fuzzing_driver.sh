...
{
   # Display Help
   echo "Syntax: ./fuzzing_driver <signature-chosen> <time-to-fuzz> <input-dir> <output-dir> <read-from-file[0|1]> <AFL_DEBUG[0|1]> <parallel-fuzzing[0|N]>" 
   echo 
...
Fuzz()
{
	...
	while read LINE; do
		...
		for lib in $(pwd)/$APP_PATH/lib/arm64-v8a/*.so ; do
			if [[ $(readelf -s $lib | grep $METHOD) ]] ; then
