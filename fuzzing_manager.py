...
def check():
    ...
    for device_id in get_device_ids():
        ...
        if not execute_privileged_command("ps -ef | grep afl-fuzz | grep -v grep | grep -v timeout", device_id=device_id)[0]:
            STILL_RUNNING = False
        else:
            STILL_RUNNING = True
...
def kill_fuzzer(target):
    ...
    for device_id in TARGETS:
        print(f"{GREEN}[LOG] {NC}Fetching PIDs for device {device_id}...")
    ...
    if args.action == "fuzz_signature" or args.action == "fuzz_one":
        ...
        if (not time_pattern.match(args.fuzz_time)):
            ...
            if args.action == "fuzz_signature":
                fuzz_signature(args.target, args.fuzz_time, args.from_file, args.parallel_fuzzing)
            else:
                fuzz_one(args.target, args.fuzz_time, args.from_file, args.parallel_fuzzing)
